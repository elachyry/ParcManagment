        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-4 trash">
                    <a href="{{route('reparation.index')}}" class="btn btn-secondary"><i
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{route('reparation.restore',$item->id)}}"
                                ...
                                <form action="{{route('reparation.hard.delete',$item->id)}}" method="post"
                                    onsubmit="return confirm('Voulez-vous supprimer définitivement ce Réparation?')">
                                    @csrf
                                    @method('HEAD')
                                    <button type="submit"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
...
$('body').on('keyup', '#search', function() {
    ...
    $.ajax({
        method: 'POST',
        url: '{{route("reparation.searchTrash")}}',
        dataType: 'json',
        data: {
            '_token': '{{ csrf_token()}}',
        ...
        success: function(res) {
            ...
            if (Array.isArray(res) && res.length == 0) {
                ...
                $.each(res, function(index, value) {
                    var url = '{{ route("reparation.restore", ":id") }}';
                    url = url.replace(':id', value.id);
                    var url1 = '{{ route("reparation.hard.delete", ":id") }}';
                    url1 = url1.replace(':id', value.id);

                    var url2 =
                        "return confirm('Voulez-vous supprimer définitivement cette Réparation?')";
