...
<main id="main" class="main">
    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Tout les Réparations</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    <br>
    ...
    <div class="jumbotron">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <a href="{{route('reparation.create')}}" class="btn btn-primary"><i
                            class="bi bi-plus-circle me-1"></i>Ajouter Réparation</a>
                            <a href="{{route('reparation.export')}}" class="btn btn-success"><i
                            class="bi bi-file-excel me-1"></i>Exporter</a>
                    <a href="{{route('reparation.trash')}}" class="btn btn-warning trash"><i
            ...
            <table class="table table-responsive-md  table-hover">
                <thead>
                    <tr
                        ...
                        <th class="px-4 py-3">
                            <center>Date Réparation</center>
                        </th>
                        <th class="px-4 py-3">
                            <center>Désignation</center>
                        </th>
                        <th class="px-4 py-3">
                            <center>Quantité</center>
                        </th>
                        <th class="px-4 py-3">
                            <center>Prix UT</center>
                        </th>
                        <th class="px-4 py-3">
                            <center>Affectaion</center>
                        </th>
                        <th class="px-4 py-3">
                            <center>Prix TTC</center>
                        </th>
                        <th class="px-4 py-3">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" id="search-result">
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3 text-sm">
                            <center>{{$item->date_reparation}}</center>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <center>{{$item->designation}}</center>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <center>{{$item->quantity}}</center>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <center>{{$item->unit_price}}</center>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <center>{{$item->affectaion}}</center>
                        </td>
                        <td class="px-4 py-3 text-sm">
                        ...
                        <td class="px-4 py-3" style="text-align:center">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{route('reparation.show',$item->id)}}"
                                ...
                                <a href="{{route('reparation.edit',$item->id)}}"
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                    aria-label="Edit">
                                ...
                                <a href="{{route('reparation.soft.Delete',$item->id)}}"
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                    aria-label="Delete">
        ...
        success: function(res) {
            ...
            if (Array.isArray(res) && res.length == 0) {
                ...
                $.each(res, function(index, value) {
                    ...
                    var url = '{{ route("reparation.show", ":id") }}';
                    url = url.replace(':id', value.id);
                    var url1 = '{{ route("reparation.edit", ":id") }}';
                    url1 = url1.replace(':id', value.id);
                    var url2 = '{{ route("reparation.soft.Delete", ":id") }}';
                    ...
                    searchResult = '<tr class="text-gray-700 dark:text-gray-400">' +
                        ...
                        '</td>' +
                        '<td class="px-4 py-3 text-sm">' +
                        '<center>' + value.date_reparation + '</center>' +
                        '</td>' +
                        '<td class="px-4 py-3 text-sm">' +
                        '<center>' + value.designation + '</center>' +
                        '</td>' +
                        '<td class="px-4 py-3 text-sm">' +
                        '<center>' + value.quantity + '</center>' +
                        '</td>' +
                        '<td class="px-4 py-3 text-sm">' +
                        '<center>' + value.unit_price + '</center>' +
                        '</td>' +
                        '<td class="px-4 py-3 text-sm">' +
                ...
                var url4 = '{{ route("reparation.exportSearch", ":id") }}';
